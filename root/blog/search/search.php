...
class blog_search
{
	...
	*/
	function save_ids($search_key, $keywords, $author_ary, $result_count, &$id_ary, $start, $sort_dir)
	{
		...
		if (!($store = $cache->get('_blog_search_results_' . $search_key)))
		{
			...
			if (!empty($keywords) || sizeof($author_ary))
			{
				$sql = 'SELECT search_time
					FROM ' . BLOG_SEARCH_RESULTS_TABLE . '
				...
				if (!$db->sql_fetchrow($result))
				{
					...
					$sql = 'INSERT INTO ' . BLOG_SEARCH_RESULTS_TABLE . ' ' . $db->sql_build_array('INSERT', $sql_ary);
					$db->sql_query($sql);
				}
		...
		if (is_array($store_ids))
		{
			...
			$cache->put('_blog_search_results_' . $search_key, $store, $config['search_store_results']);

			$sql = 'UPDATE ' . BLOG_SEARCH_RESULTS_TABLE . '
	...
	*/
	function destroy_cache($words, $authors = false)
	{
		...
		if (sizeof($words))
		{
			...
			{
				$cache->destroy('_blog_search_results_' . $row['search_key']);
			}
			$db->sql_freeresult($result);
		}
