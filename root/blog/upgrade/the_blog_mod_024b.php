...
$this->available_upgrades[$name]['custom_options'] = array(
	...
	'convert_friend_foe'	=> array('lang' => 'TBM_CONVERT_FRIEND_FOE',	'type' => 'radio:yes_no',	'explain' => true,		'default' => true),
);
...
* 1. Upgrade Blogs
* 2. Upgrade everything else
*/
if (isset($run_blog_upgrade) && $run_blog_upgrade)
{
	...
	$cnt = 0;
	$sql = 'SELECT * FROM ' . $this->selected_options['db_prefix'] . 'weblog_entries ORDER BY entry_time ASC';
	$result = $old_db->sql_query($sql);
	while ($row = $old_db->sql_fetchrow($result))
	{
		...
		$cnt++;
		decode_message(utf8_normalize_nfc($row['entry_text']), $row['bbcode_uid']);
		...
		$sql = 'INSERT INTO ' . BLOGS_TABLE . ' ' . $db->sql_build_array('INSERT', $sql_array);
		$db->sql_query($sql);
	...
	if ($cnt >= $limit)
	{
		...
		$start += $limit;
		$part_message = sprintf($user->lang['BREAK_CONTINUE_NOTICE'], $stage, ($start /$limit));
	}
}


if (isset($run_remaining_upgrade) && $run_remaining_upgrade)
{
	if (!$this->selected_options['truncate'])
	{
	...
	{
		$sql = 'SELECT * FROM ' . $this->selected_options['db_prefix'] . 'weblog_replies ORDER BY post_time ASC';
		$result = $old_db->sql_query($sql);
		while ($row = $old_db->sql_fetchrow($result))
		{
			if ($cnt < $start || $cnt >= $limit)
			...
			$cnt++;
			decode_message(utf8_normalize_nfc($row['reply_text']), $row['bbcode_uid']);
			$message_parser = new parse_message();
			$message_parser->message = $text;
			$message_parser->parse($row['enable_bbcode'], 1, $row['enable_smilies']);
			...
			$sql = 'INSERT INTO ' . BLOGS_REPLY_TABLE . ' ' . $db->sql_build_array('INSERT', $sql_array);
			$db->sql_query($sql);
	...
	{
		if ($this->selected_options['convert_friend_foe'])
		{
