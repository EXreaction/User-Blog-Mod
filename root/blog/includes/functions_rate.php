...
function get_star_rating($int_rating, $blog_id, $average = false)
{
	...
	{
		$final_code .= ($user->data['is_registered'] && !$average && !isset($rating_data[$blog_id])) ? '<a href="' . blog_url(false, $blog_id, false, array('page' => 'rate', 'rating' => $i, 'c' => (($category_id) ? $category_id : '*skip*'))) . '">' : '';
		...
		$final_code .= ($user->data['is_registered'] && !$average && !isset($rating_data[$blog_id])) ? "onmouseover=\"ratingHover('{$i}', '{$unique_str}')\"  onmouseout=\"ratingUnHover('{$int_rating}', '{$unique_str}')\"  onmousedown=\"ratingDown('{$i}', '{$unique_str}')\"" : '';
		...
		$final_code .= ($user->data['is_registered'] && !$average && !isset($rating_data[$blog_id])) ? '</a>' : '';
	}
